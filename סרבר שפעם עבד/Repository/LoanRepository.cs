...
namespace Repository
{
    public class LoanRepository : ILoanRepository
    {
        ...
        {
            var query = _charityContext.Loans.Where(loan =>
                (name == null ? (true) : loan.ItemName.Contains(name))
                && (email == null ? (true) : loan.BorrowerEmail.Contains(name))
                && (borrowName == null ? (true) : loan.BorrowerName.Contains(name))
                )
                .OrderBy(Loan => Loan.LoanId);
            Console.WriteLine(query.ToQueryString());
            IEnumerable<Loan> loans = await query.ToListAsync();
            return loans;
